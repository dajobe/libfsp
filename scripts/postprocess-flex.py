#!/usr/bin/python3
"""
Post-process files generated by flex to eliminate warnings and improve compatibility.
...
Usage:
  ...
  # a) Write to stdout
  python3 postprocess-flex.py lexer.c > lexer.c.tmp
  mv lexer.c.tmp lexer.c

  # b) Write the file directly
  python3 postprocess-flex.py -o lexer.c.tmp lexer.c

Process multiple files:
  for file in lexer.c lexer.h; do
    python3 postprocess-flex.py $file -o $file.tmp
    mv $file.tmp $file
  done

Host projects should specify their config header:
...
"""
            ...
            while s:
                ...
                # definition
                if re.match(
                    r"^static void( yynoreturn)? yy_fatal_error\s*\(.*\)\s*[^\;]\s*$",
                    line,
                ):
...
"""
                ...
                if cur_function == f"{prefix}_get_next_buffer":
                    if "seen_yyinput" not in fn_state and re.match(
                        r"^\s*YY_INPUT\(", line
                    ):
                ...
                line = re.sub(
                    r"^\#line \d+ (.*\.[ch])",
                    rf"#line {line_number + line_offset} \1",
                    line,
                )
